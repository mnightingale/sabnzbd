...
class TestDownloadFlow(DownloadFlowBasics):
    ...
    def test_download_zip(self):
        self.download_nzb("test_zip", ["My_Test_Download.bin"], True)

    def test_download_7zip(self):
        self.download_nzb("test_7zip", ["My_Test_Download.bin"], True)

    def test_download_passworded(self):
        self.download_nzb("test_passworded{{secret}}", ["My_Test_Download.bin"], True)

    def test_download_unicode_made_on_windows(self):
        self.download_nzb("test_win_unicode", ["Frènch_German_Demö.bin"], True)

    def test_download_fully_obfuscated(self):
        # This is also covered by a unit test but added to test full flow
        self.download_nzb("obfuscated_single_rar_set", ["My_Test_Download.bin"], True)

    def test_download_unicode_rar(self):
        self.download_nzb("unicode_rar", ["我喜欢编程_My_Test_Download.bin"], True)
