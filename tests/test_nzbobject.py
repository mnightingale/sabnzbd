...
from sabnzbd.filesystem import globber, sanitize_filename, same_directory
...
class TestNZO:
    ...
    def test_get_unique_filepath_subdirs(self):
        ...
        # A repeat stays in the same folder and only the name is made unique
    ...
    @pytest.mark.config({"download_dir": SAB_CACHE_DIR})
    def test_get_unique_filepath_cannot_escape_download_path(self):
        """Sanitizing strips the traversal, so nothing can be written outside the job folder"""
        ConfigCat("*", {"pp": 3, "script": "None", "priority": NORMAL_PRIORITY})
        nzo = NzbObject("test_no_escape", nzb_fp=create_and_read_nzb_fp("basic_rar5"))

        for hostile in ("../escape.rar", "/escape.rar", "sub/../../escape.rar", "../../../etc/escape.rar"):
            _filename, path = nzo.get_unique_filepath(sanitize_filename(hostile, allow_subdirs=True))
            assert same_directory(nzo.download_path, path) == 2, "%s escaped to %s" % (hostile, path)
