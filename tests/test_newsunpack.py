...
class TestUnpackNestedRarSets:
    ...
    def _create_job(tmp_path, subdir: str = "") -> tuple[mock.Mock, str]:
        ...
        nzo = mock.Mock(
            ...
            # As NzbObject.relocated_directories() would report it after the quick-check rename
            relocated_directories=mock.Mock(return_value={os.path.join(download_path, subdir)} if subdir else set()),
        )
    ...
    def test_relocated_directories_reports_only_local_subfolders(self, tmp_path):
        """Both the quick-check renames (nzf.filename) and those done by par2cmdline (nzo.renames)"""
        ...
        assert NzbObject.relocated_directories(nzo) == {
