...
class Assembler(Thread):
        ...
        if nzf is None:
            ...
            if (
                ...
                with self.queued_lock:
                    ...
                    self.queue.put(AssemblerTask(nzo, nzf, file_done, force))
    ...
    def run(self):
        while 1:
            ...
            nzo = nzf = None
            nzo, nzf, file_done, force = self.queue.get()
            if not nzo:
            ...
            if nzf:
                ...
                if filepath := nzf.prepare_filepath():
                    try:
                        ...
                        self.assemble(nzo, nzf, file_done, force)
    ...
    @staticmethod
    def assemble(nzo: NzbObject, nzf: NzbFile, file_done: bool, force: bool) -> None:
    ...
    @staticmethod
    def assemble_article(article: Article, data: bytes) -> bool:
    ...
    @staticmethod
    def write_at_offset(fd: int, nzf: NzbFile, article: Article, data: bytes):
        mv = memoryview(data)
        written = 0
        while written < len(data):
