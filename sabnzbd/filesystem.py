...
class WriteMonitor:
    ...
    def reset(self, path: Optional[str] = None):
        ...
        self.hint = is_rotational(self.path) if self.path else None
        self.allow_direct_decode = self.hint is not True
        self.retry_after = RETRY_AFTER
    ...
    @synchronized()
    def allows_direct_decode(self) -> bool:
        """May articles be written straight to their offset as they arrive?"""
        return self.allow_direct_decode
    ...
    def sample(self):
        ...
        if written < MIN_COST_BYTES or nanos <= 0:
            return
    ...
    def log_profile(self):
        ...
        logging.info("Storage profile for %s: %s", self.path, self)
