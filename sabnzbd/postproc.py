...
def parring(nzo: NzbObject) -> tuple[bool, bool]:
    ...
    if not par_error:
        ...
        _, rars, _, _, _ = build_filelists(nzo.download_path, check_rar=False, extra_dirs=nzo.relocated_directories())
        if not rars:
        ...
        if not any(verified.values()):
            ...
            if sfv_check_result is None and cfg.enable_unrar():
                ...
                _, rars, _, _, _ = build_filelists(
                    nzo.download_path, check_rar=False, extra_dirs=nzo.relocated_directories()
                )
                if rars:
