...
class RSSReader:
    ...
    def run_feed(
        ...
        # Fetch & parse RSS
        entries, msg = self.fetch_rss(feed, uris, readout)

        # Decide which entries to work with
        if not readout:
            entries = jobs
        ...
        for entry in entries:
            ...
            try:
                if readout:
                    ...
                    if len(uris) > 1 and normalised.is_duplicate(jobs):
                        continue
                else:
    ...
    @staticmethod
    def fetch_rss(feed: str, uris: list[str], readout: bool) -> tuple[list[str], str]:
...
class NormalisedEntry:
    ...
    def _get_link(
        ...
        Returns (link, infourl, category, size, age, season, episode)
        """
