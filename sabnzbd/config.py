...
def create_config_backup() -> str | bool:
    ...
    from sabnzbd.database import history_db_snapshot, HistoryDB
        ...
        with open(complete_path, "wb") as zip_buffer:
            with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_ref:
                for filename in CONFIG_BACKUP_FILES:
                    ...
                    snapshot = None
                    if filename == DB_HISTORY_NAME and HistoryDB.db_path and full_path == HistoryDB.db_path:
                        snapshot = history_db_snapshot()
                    if snapshot:
